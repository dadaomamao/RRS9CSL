...
- 将 `HighPriorityCNRealDNSDirect.list` 中微博、米哈游 / 原神 / 米游社、抖音 / 字节视频娱乐域名从注释候选重新恢复为生效规则，用于最后一轮高频境内 real-DNS DIRECT 路径验证。
- 本轮本省网络优化工程已进入投产前变更冻结与全量回归验收阶段，本次是最后的本省运营商接入侧链路验证窗口；变更后须按全量回归口径执行，覆盖 DNS 解析、CDN 就近调度、real-DNS DIRECT 命中、代理绕行、应用登录和高频业务路径。
- 安全运维验收口径以变更冻结和风险收敛为主：重点观察域名分流漂移、策略组误命中、运营商 DNS 回退、缓存污染、跨规则集覆盖冲突和异常回源；全量测试闭环前，不把本轮结果视为生产稳定结论。
- 按 `AGENTS.md` 完成全仓库防御性红蓝审查：9 个 `.list` 文件共 1288 条生效规则，语法坏行 0，代理 / 直连 exact 冲突 0，代理 / 直连覆盖冲突 0；本次 diff 未发现凭据、密钥、攻击性测试指令、提示注入、主机配置修改、第三方上游引入或敏感金融 / Claude / Anthropic 归属变更。

## 2026-06-10

### DNS upstream usage boundary

- 在 `docs/USAGE.md` 增补 DNS 解析上游说明：境外域名只能使用 Google 加密 DNS，不能用明文 `8.8.8.8` / `8.8.4.4` 替代。
- 明确境内域名必须使用本地运营商 DNS 和大厂公共 DNS 并发解析，避免国内 CDN 调度失真导致尾部延迟极大。
- 本次只同步 Markdown 使用说明和维护记录，不修改任何 `.list` 规则行。
