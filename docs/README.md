...
1. 修改任何文件前，必须先读 [AGENTS.md](../AGENTS.md)。
2. 规则文件保持小而明确，只放当前实际需要的补充项。
3. 第三方规则只通过 URL 引用，不复制成新的上游规则库。
4. 对已有用户，根目录规则文件名和 raw URL 是稳定接口；未经明确确认不重命名。
5. 完全相同的规则不要在多个列表里重复出现；确需重复时必须在文档里写清原因。
